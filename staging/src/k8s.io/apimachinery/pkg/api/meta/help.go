	...
	for i := 0; i < itemsLen; i++ {
		...
		case raw.Type() == rawExtensionObjectType:
			...
			case item.Raw != nil:
				...
				if err := fn(&runtime.Unknown{Raw: item.Raw}); err != nil {
					return err
				}
			default:
		...
		default:
			if !reflect.PointerTo(raw.Type()).Implements(objectType) {
				return fmt.Errorf("%v: item[%v]: Expected object, got %#v(%s)", items.Interface(), i, raw.Interface(), raw.Kind())
			}
			if err := fn(raw.Addr().Interface().(runtime.Object)); err != nil {
				return err
			}
		}
	}
...
func EachListItemWithAlloc(obj runtime.Object, fn func(runtime.Object) error) error {
	...
	for i := 0; i < itemsLen; i++ {
		...
		default:
			...
			new.Elem().Set(raw)
			if err := fn(new.Interface().(runtime.Object)); err != nil {
				return err
			}
		}
	}
